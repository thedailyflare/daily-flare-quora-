...
class MainActivity : Activity() {
 ...
 private lateinit var storyScroll:ScrollView
 ...
 private fun render(){
  ...
  top.addView(tv("LIVE",10f,green,true).apply{gravity=Gravity.CENTER;background=shape(Color.rgb(228,239,232),14);setPadding(dp(10),dp(6),dp(10),dp(6))},LinearLayout.LayoutParams(dp(58),dp(30)))
  head.addView(top)
  head.addView(tv("SOCIAL SHARING DESK",11f,gold,true).apply{letterSpacing=.12f})
  ...
  storyScroll=ScrollView(this)
  list=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL}
  storyScroll.addView(list)
  swipeRefresh=SwipeRefreshLayout(this).apply{setOnRefreshListener{loadFeed()};addView(storyScroll)}
 ...
 private fun loadFeed(page:Int=1,append:Boolean=false){
  ...
  else{currentPage=1;totalPages=1;if(::swipeRefresh.isInitialized)swipeRefresh.isRefreshing=true;list.removeAllViews();loadMoreButton.visibility=View.GONE}
  Thread{
   try{
    ...
    loadMoreButton.isEnabled=true
    loadMoreButton.text="Load 10 more stories"
    if(append){Toast.makeText(this@MainActivity,"Couldn't load more stories",Toast.LENGTH_LONG).show();filterStories(searchBox.text?.toString().orEmpty())}
    else{list.addView(errorCard())}
   }}
  }.start()
 }
 ...
 private fun displayDateTime(iso:String):Pair<String,String>{
  return try{
   ...
   val dateFormat=SimpleDateFormat("dd-MM",Locale.US).apply{timeZone=TimeZone.getDefault()}
 ...
 private fun addCard(story:Story,number:Int){
  ...
  val dt=displayDateTime(story.date)
  meta.addView(tv(dt.first,10f,gold,true).apply{letterSpacing=.08f},LinearLayout.LayoutParams(0,dp(28),1f))
  val timeChip=tv(dt.second,11f,navy,true).apply{gravity=Gravity.CENTER;background=shape(Color.rgb(239,242,244),13);setPadding(dp(10),dp(4),dp(10),dp(4))}
  meta.addView(timeChip,LinearLayout.LayoutParams(dp(64),dp(28)))
  val done=posted.getBoolean(story.link,false)
  if(done)meta.addView(tv("POSTED ✓",10f,green,true).apply{background=shape(Color.rgb(228,239,232),12);setPadding(dp(9),dp(5),dp(9),dp(5))}.also{it.layoutParams=LinearLayout.LayoutParams(-2,dp(28)).apply{leftMargin=dp(7)}})
  ...
  meta.addView(titleCopy,LinearLayout.LayoutParams(dp(36),dp(30)).apply{leftMargin=dp(4)});card.addView(meta)
  card.addView(tv(story.title,19f,ink,true).apply{setPadding(0,dp(8),0,dp(14));maxLines=3})
  ...
  addSocial(row1,socialIcon("f",Color.rgb(24,119,242),"facebook"){
   ...
   Toast.makeText(this,"Facebook text copied — paste it into your post",Toast.LENGTH_LONG).show()
 ...
 data class Story(val title:String,val link:String,val excerpt:String,val tags:List<String>,val date:String)
}